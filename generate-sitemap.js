...
async function generate() {
  ...
  // SEO landing pages (city/category/sector) — generated by build-seo-pages.js
  let seoPages = [];
  try {
    const slugs = JSON.parse(fs.readFileSync('.seo-pages.json', 'utf8'));
    seoPages = slugs.map(s => ({ loc: `https://taleempk.pk/${s}`, lastmod: today, priority: '0.8', changefreq: 'weekly' }));
  } catch (e) { console.warn('  (no .seo-pages.json — run build-seo-pages.js first to include landing pages)'); }

  const allPages = [...staticPages, ...seoPages, ...blogPages, ...uniPages];
...
${allPages.map(p => `  <url><loc>${p.loc}</loc><lastmod>${p.lastmod}</lastmod><changefreq>${p.changefreq}</changefreq><priority>${p.priority}</priority></url>`).join('\n')}
  ...
  console.log(`sitemap.xml generated with ${allPages.length} URLs (${staticPages.length} static + ${seoPages.length} landing + ${blogPages.length} blog + ${uniPages.length} unis)`);
}
